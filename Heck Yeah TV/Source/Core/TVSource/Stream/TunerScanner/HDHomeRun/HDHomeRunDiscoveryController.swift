...
final class HDHomeRunDiscoveryController {
    ...
    private func deviceDetails() async -> FetchSummary {
        ...
        for device in discoveredDevices {
            let discoverURL = device.discoverURL
            ...
            do {
                ...
                summary.successes[discoverURL] = 1
            } catch {
                summary.failures[discoverURL] = error
            }
        }
    ...
    private func tunerChannelLineUp(_ tunerServerDevice: HDHomeRunDevice) async -> FetchSummary {
        var summary = FetchSummary()
    ...
    func bootStrapTunerChannelDiscovery() async -> FetchSummary {
        ...
        if let tuner = devices.first {
            summary.mergeSummary(await self.tunerChannelLineUp(tuner))
        }
        return summary
    }
}
